...
void RnaMatcher::setSubstitutionMatrix(BaseMatrix *m){
...
void RnaMatcher::initQuery(Sequence* query){
    currentQuery = query;
    if(Parameters::isEqualDbtype(query->getSeqType(), Parameters::DBTYPE_HMM_PROFILE)){
...
RnaMatcher::result_t RnaMatcher::getSWResult(Sequence* dbSeq, const int, bool isReverse, const int covMode, const float covThr,
    ...
    // no valid alignment
    if (alignment.dbEndPos1 == -1) {
        return result_t(dbSeq->getDbKey(), 0, 0.0f, 0.0f, 0.0f, DBL_MAX, 0,
                        -1, -1, origQueryLen, -1, -1, dbSeq->L, "");
    }
...
//    alignment->score1 = alignment->score1 - log2(dbSeq->L);
    ...
    if(alignmentMode == RnaMatcher::SCORE_COV_SEQID){
        // compute sequence id
        if (backtrace.size() > 0) {
...
void RnaMatcher::readAlignmentResults(std::vector<result_t> &result, char *data, bool readCompressed) {
    if(data == NULL) {
        return;
    }
...
int RnaMatcher::computeAlnLength(int qStart, int qEnd, int dbStart, int dbEnd) {
    return std::max(abs(qEnd - qStart), abs(dbEnd - dbStart)) + 1;
}

float RnaMatcher::estimateSeqIdByScorePerCol(uint16_t score, unsigned int qLen, unsigned int tLen) {
...
std::string RnaMatcher::compressAlignment(const std::string& bt) {
    std::string ret;
    char state = 'M';
    size_t counter = 0;
...
std::string RnaMatcher::uncompressAlignment(const std::string &cbt) {
    std::string bt;
    bt.reserve(cbt.size());
    size_t count = 0;
...
RnaMatcher::result_t RnaMatcher::parseAlignmentRecord(const char *data, bool readCompressed) {
    ...
    double qCov = RnaSmithWaterman::computeCov(adjustQstart, qEnd, qLen);
    double dbCov = RnaSmithWaterman::computeCov(adjustDBstart, dbEnd, dbLen);
    size_t alnLength = RnaMatcher::computeAlnLength(adjustQstart, qEnd, adjustDBstart, dbEnd);

    switch(columns) {
        // 10 no backtrace
        case ALN_RES_WITHOUT_BT_COL_CNT:
            return RnaMatcher::result_t(targetId, score, qCov, dbCov, seqId, eval,
...
size_t RnaMatcher::resultToBuffer(char * buff1, const result_t &result, bool addBacktrace, bool compress, bool addOrfPosition) {
    char * basePos = buff1;
    ...
    *(tmpBuff-1) = '\t';
    ...
    if(addBacktrace == true){
        if(compress){
            *(tmpBuff-1) = '\t';
            std::string compressedCigar = RnaMatcher::compressAlignment(result.backtrace);
...
void RnaMatcher::updateResultByRescoringBacktrace(const char *querySeq, const char *targetSeq, const char **subMat, EvalueComputation &evaluer,
    ...
    double evalue = rnaEvalueCorrection(evaluer.computeEvalue(maxScore, result.qLen));
