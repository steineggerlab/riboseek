...
/* MMseqs bridge hook: optionally return calibrated CM text in-memory
 * instead of rewriting cfg->cmfile on disk.
 */
static int    g_mmseqs_capture_cm_to_memory = 0;
static char  *g_mmseqs_captured_cm_text     = NULL;
static size_t g_mmseqs_captured_cm_text_len = 0;

static void
infernal_cmcalibrate_set_capture_to_memory_internal(int enabled)
{
  g_mmseqs_capture_cm_to_memory = enabled ? 1 : 0;
  if (g_mmseqs_captured_cm_text != NULL) {
    free(g_mmseqs_captured_cm_text);
    g_mmseqs_captured_cm_text = NULL;
  }
  g_mmseqs_captured_cm_text_len = 0;
}

static const char *
infernal_cmcalibrate_get_captured_cm_text_internal(size_t *opt_len)
{
  if (opt_len != NULL) *opt_len = g_mmseqs_captured_cm_text_len;
  return g_mmseqs_captured_cm_text;
}
...
main(int argc, char **argv)
{
  ...
  if(cfg.my_rank == 0 && 
    ...
    CM_t *cm;
    FILE *outfp;
    char *mem_cm_text = NULL;
    size_t mem_cm_text_len = 0;
    sigset_t blocksigs;  /* list of signals to protect from */

    cm_file_Position(cfg.cmfp, (off_t) 0);

    if (g_mmseqs_capture_cm_to_memory) {
      if ((outfp = open_memstream(&mem_cm_text, &mem_cm_text_len)) == NULL) cm_Fail("Ouch. open_memstream() failed.");
    } else {
    ...
    if (g_mmseqs_capture_cm_to_memory) {
      if (g_mmseqs_captured_cm_text != NULL) free(g_mmseqs_captured_cm_text);
      g_mmseqs_captured_cm_text = mem_cm_text;
      g_mmseqs_captured_cm_text_len = mem_cm_text_len;
      mem_cm_text = NULL;
      mem_cm_text_len = 0;
    } else {
