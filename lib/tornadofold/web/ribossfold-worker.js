...
// The main thread passes the module to load (?m=ribossfold[-simd].js) so the
// worker matches the page's SIMD-support decision.
const module = new URLSearchParams(self.location.search).get("m") || "ribossfold.js";
importScripts(module);

const ready = ribossfold();
