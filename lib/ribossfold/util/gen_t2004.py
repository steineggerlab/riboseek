...
base index bidx(N)=0, A..U=1..4; int22 is [7][7][5][5][5][5] (including N).
...
# --- re-index ViennaRNA [8]/[5] arrays into t2004.h [7]/[5] layout -----------
def _get(arr, idx):
    for i in idx:
        arr = arr[i]
    return arr
...
def fill_N(arr, shape, base_axes):
    """Fill N (index 0) base slots with ViennaRNA's non-standard-base rule"""
    for idx in itertools.product(*(range(s) for s in shape)):
        npos = [ax for ax in base_axes if idx[ax] == 0]
        if not npos:
            continue
        best = None
        for sub in itertools.product(range(1, 5), repeat=len(npos)):
            cc = list(idx)
            for ax, s in zip(npos, sub):
                cc[ax] = s
            val = _get(arr, cc)
            if val < VIE_INF:
                best = val if best is None else max(best, val)
        if best is not None:
            _set(arr, list(idx), best)
    return arr
...
def reindex(v):
    ...
    t["int22"] = [[[[[[v["int22"][a + 1][b + 1][w][x][y][z]
                       for z in range(5)] for y in range(5)] for x in range(5)]
                    for w in range(5)] for b in range(7)] for a in range(7)]
    for k in ("hairpin", "bulge", "internal", "tri", "tetra", "hexa", "ML_closing",
              "ML_intern", "ML_BASE", "NINIO_max", "NINIO_m", "TerminalAU", "lxc"):
        t[k] = v[k]
    # Fill N (index 0) base slots.
    for name in ("mm_hairpin", "mm_internal", "mm_internal_1n", "mm_internal_23",
                 "mm_multi", "mm_exterior"):
        fill_N(t[name], (7, 5, 5), base_axes=(1, 2))
    for name in ("dangle5", "dangle3"):
        fill_N(t[name], (7, 5), base_axes=(1,))
    fill_N(t["int11"], (7, 7, 5, 5), base_axes=(2, 3))
    fill_N(t["int21"], (7, 7, 5, 5, 5), base_axes=(2, 3, 4))
    fill_N(t["int22"], (7, 7, 5, 5, 5, 5), base_axes=(2, 3, 4, 5))
    return t
...
CUR_SHAPES = {
    ...
    "int11": (7, 7, 5, 5), "int21": (7, 7, 5, 5, 5), "int22": (7, 7, 5, 5, 5, 5),
...
def canon(name, arr):
    ...
    if name in ("hairpin", "bulge", "internal"):
        return arr[3:31]
    if name == "int22":
        return [arr[a][b][w][x][y][z] for a in P for b in P
                for w in B for x in B for y in B for z in B]
    return list(flat(arr))
...
def emit_t2004(new, lxc):
    L = [
        ...
        "// bidx(N)=0, A..U=1..4. N base slots hold ViennaRNA's max-substitution",
        "// penalty; sentinel (NN pair) entries are unreachable and set to 0. lxc",
        "// is kept at ViennaRNA's",
    ...
    L.append(f"static const int16_t int22[7][7][5][5][5][5] = {cflat(new['int22'])};")
