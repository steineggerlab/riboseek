...
void setSplitSearchDefaults(Parameters *p) {
    p->spacedKmer = true;
...
int computeSplitSearchMode(int queryDbType, int targetDbType, int targetSrcDbType, int searchType) {
...
void setSplitNuclSearchDefaults(Parameters *p) {
...
int splitsearch(int argc, const char **argv, const Command& command) {
    Parameters &par = Parameters::getInstance();
    setSplitSearchDefaults(&par);
    ...
    int searchMode = computeSplitSearchMode(queryDbType, targetDbType, targetSrcDbType, par.searchType);
    // if ((searchMode & Parameters::SEARCH_MODE_FLAG_QUERY_NUCLEOTIDE) && (searchMode & Parameters::SEARCH_MODE_FLAG_TARGET_NUCLEOTIDE)) {
    setSplitNuclSearchDefaults(&par);
    // } else{
...
//        }
    ...
    std::string tmpDir = par.filenames[par.filenames.size() - 1];
...
//                par.queryProfile = true;
        ...
        FileUtil::writeFile(tmpDir + "/blastsplitp.sh", blastsplitp_sh, blastsplitp_sh_len);
        program = std::string(tmpDir + "/blastsplitp.sh");
    }
    ...
    if (program.find("blastsplitdigp") == std::string::npos && program.find("blastsplitdigp2")) {
        ...
        par.forceCompBiasCorrection = 1;
        par.remapProfile = true;
        FileUtil::writeFile(tmpDir + "/blastsplitdi.sh", blastsplitdi_sh, blastsplitdi_sh_len);
        cmd.addVariable("EXTRACTQUERYPROFILES","TRUE");
        cmd.addVariable("SPLITSEQUENCE_PAR", par.createParameterString(par.splitsequence).c_str());
        if(indexStr=="") {
            cmd.addVariable("NEEDTARGETSPLIT", "TRUE");
        ...
        program = std::string(tmpDir + "/blastsplitdi.sh");
    }
