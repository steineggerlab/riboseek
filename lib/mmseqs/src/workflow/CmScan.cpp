...
static bool cmRefineTraceEnabled() {
    ...
    if (cached == -1) {
        ...
        if (env != NULL) {
            ...
            // In fast mode, skip refine traceback by default for speed.
            cached = (cmFastMathEnabled() || cmFastLogsumEnabled()) ? 0 : 1;
        }
    }
    return cached == 1;
}
...
static inline float bSplitBest(const float *left, const float *rightStart, int rightStep, int kBeg, int kEnd, float negInf) {
    float best = negInf;
    int k = kBeg;
#if defined(__AVX2__)
...
loadResultDbCandidates(const std::string &resultDbPath, int threads) {
    std::unordered_map<unsigned int, std::unordered_set<unsigned int> > out;
    const std::string idx = resultDbPath + ".index";
    // If the prefilter result DB is absent, return empty map → caller falls back to all-vs-all.
    if (!FileUtil::fileExists(resultDbPath.c_str()) || !FileUtil::fileExists(idx.c_str())) {
        return out;
    }
    ...
    out.reserve(rdb.getSize() * 2 + 1);
    for (size_t i = 0; i < rdb.getSize(); ++i) {
        const unsigned int qKey = rdb.getDbKey(i);
        std::unordered_set<unsigned int> &cand = out[qKey];
        ...
        while (*data != '\0') {
            ...
            if (endptr != data && k <= static_cast<unsigned long>(UINT_MAX)) {
                cand.insert(static_cast<unsigned int>(k));
            }
...
static std::vector<FastaSeq> readSequenceDb(const std::string &path) {
    const std::string indexPath = path + ".index";
    DBReader<unsigned int> dbr(path.c_str(),
                               indexPath.c_str(),
                               1,
    ...
    std::vector<FastaSeq> seqs;
    seqs.reserve(dbr.getSize());
    NucleotideMatrix subMat(Parameters::getInstance().scoringMatrixFile.values.nucleotide().c_str(), 1.0, 0.0);
    Sequence seqObj(dbr.getMaxSeqLen(), Parameters::DBTYPE_NUCLEOTIDES, &subMat, 0, false, false);
    const bool hasLookup = (dbr.getLookupSize() > 0);
    for (size_t i = 0; i < dbr.getSize(); ++i) {
        FastaSeq cur;
        cur.key = dbr.getDbKey(i);
        if (hasLookup) {
        ...
        const size_t seqLen = dbr.getSeqLen(i);
        const char *data = dbr.getData(i, 0);
        seqObj.mapSequence(i, cur.key, data, seqLen, false, NULL, false, false, 1.0f, true);
...
static std::string modelTraceCigar(const InfernalExactModel &model, const std::vector<int> &traceStates) {
    ...
    for (int p = minMap; p <= maxMap; ++p) {
        ...
        const char o = (mop[static_cast<size_t>(p)] == 'D') ? 'D' : 'M';
        ops.push_back(o);
...
static double exactVitRec(ExactRecCtx &ctx, int v, int i, int j) {
    ...
    if (ctx.useDense) {
        ...
        if ((*ctx.memoVitSeen)[dk] == ctx.curVitGen) {
            return (*ctx.memoVitDense)[dk];
        }
    } else {
    ...
    const ExactStateExec &st = (*ctx.exec)[static_cast<size_t>(v)];
    const int d = j - i + 1;
    if (st.dmax >= st.dmin && (d < st.dmin || d > st.dmax)) {
        if (ctx.useDense) {
            const size_t dk = static_cast<size_t>(key);
            (*ctx.memoVitSeen)[dk] = ctx.curVitGen;
            (*ctx.memoVitDense)[dk] = NEG_INF;
        } else {
...
static void runInfernalExactScan(const InfernalExactModel &model,
    ...
    if (cmExactFastDeckEnabled() && N <= 1024) {
        ...
        const size_t deckStride = static_cast<size_t>(N + 1);
        const size_t stateStride = static_cast<size_t>(N + 2) * deckStride;
        ...
        float *bSplitTmp = ws.bSplitTmp.data();
        ...
        for (int v = 0; v < M; ++v) {
            ...
            for (int i = 1; i <= N + 1; ++i) {
                const size_t at = vb + static_cast<size_t>(i) * deckStride;
                vit[at] = 0.0f;
            }
        }
            ...
            for (size_t vsi = 0; vsi < activeStates.size(); ++vsi) {
                ...
                if (dmax >= dmin && (d < dmin || d > dmax)) {
                    for (int i = 1; i <= iMax; ++i) {
                        const size_t at = vBase + static_cast<size_t>(i) * deckStride + static_cast<size_t>(d);
                        vit[at] = NEG_INF_F;
                    }
                    continue;
                }
                if (isBState[vi]) {
                    ...
                    for (int i = 1; i <= iMax; ++i) {
                        ...
                        const float *leftDeck = &vit[stateBase[static_cast<size_t>(y)] + static_cast<size_t>(i) * deckStride];
                        const float *rightStart = &vit[stateBase[static_cast<size_t>(z)] +
                                                       static_cast<size_t>(i) * deckStride +
                                                       static_cast<size_t>(d)];
                        const int rightStep = static_cast<int>(deckStride) - 1;
                        if (kBeg == 0 && kEnd == d) {
                            vit[at] = bSplitBestFull(leftDeck, rightStart, rightStep, d, NEG_INF_F);
                        } else {
                            const int len = kEnd - kBeg + 1;
                            for (int kk = 0; kk < len; ++kk) {
                                const int k = kBeg + kk;
                                bSplitTmp[kk] = rightStart[k * rightStep];
                            }
                            vit[at] = bSplitBestPacked(leftDeck + kBeg, bSplitTmp, len, NEG_INF_F);
                        }
                    }
                    continue;
                }
                ...
                for (int i = 1; i <= iMax; ++i) {
                    ...
                    const size_t at = vBase + static_cast<size_t>(i) * deckStride + static_cast<size_t>(d);
                    const int ni = i + niShift;
                    const size_t ndBase = static_cast<size_t>(ni) * deckStride + static_cast<size_t>(nd);
                    float best = NEG_INF_F;
                    if (trCount == 1) {
                        const float n = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 0])] + ndBase];
                        if (n != NEG_INF_F) {
                            best = ef + nbTrScF4[t4 + 0] + n;
                        }
                    } else if (trCount == 2) {
                        const float n0 = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 0])] + ndBase];
                        const float n1 = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 1])] + ndBase];
                        if (n0 != NEG_INF_F) { best = ef + nbTrScF4[t4 + 0] + n0; }
                        if (n1 != NEG_INF_F) {
                            const float cand1 = ef + nbTrScF4[t4 + 1] + n1;
                            if (cand1 > best) {
                                best = cand1;
                            }
                        }
                        ...
                        const float n0 = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 0])] + ndBase];
                        const float n1 = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 1])] + ndBase];
                        const float n2 = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 2])] + ndBase];
                        const float n3 = vit[stateBasePtr[static_cast<size_t>(nbTrDst4[t4 + 3])] + ndBase];
                        if (n0 != NEG_INF_F) { best = ef + nbTrScF4[t4 + 0] + n0; }
                        if (n1 != NEG_INF_F) {
                            const float cand1 = ef + nbTrScF4[t4 + 1] + n1;
                            if (cand1 > best) {
                        ...
                        if (n2 != NEG_INF_F) {
                            const float cand2 = ef + nbTrScF4[t4 + 2] + n2;
                            if (cand2 > best) {
                                best = cand2;
                            }
                        }
                        if (n3 != NEG_INF_F) {
                            const float cand3 = ef + nbTrScF4[t4 + 3] + n3;
                            if (cand3 > best) {
                                best = cand3;
                            }
                        }
                        ...
                        for (int t = 0; t < trCount; ++t) {
                            ...
                            const float n = vit[stateBasePtr[static_cast<size_t>(trDstPtr[ti])] + ndBase];
                            if (n != NEG_INF_F) {
                                const float cand = ef + trScFPtr[ti] + n;
                                if (cand > best) {
                                    best = cand;
                                }
                            }
        ...
        for (int d = minSpan; d <= maxSpan; ++d) {
            ...
            for (int i = 1; i <= iMax; ++i) {
                const float rawSc = vit[rootBase + static_cast<size_t>(i) * deckStride + static_cast<size_t>(d)];
                if (rawSc == NEG_INF_F) {
                    continue;
                }
                const int j = i + d - 1;
        ...
        while (!st.empty()) {
            ...
            const double cur = vit[stateBase[static_cast<size_t>(c.v)] +
                                   static_cast<size_t>(c.i) * deckStride +
            ...
            if (sv.type == CM_ST_B) {
                ...
                for (int k = kBeg; k <= kEnd; ++k) {
                    ...
                    const float r = vit[stateBase[static_cast<size_t>(z)] +
                                        static_cast<size_t>(c.i + k) * deckStride +
                                        static_cast<size_t>(c.d - k)];
...
int cmscan(int argc, const char **argv, const Command &command) {
    ...
    std::vector<QueryModel> queryModels;
    if (hasDbIndex(par.db1)) {
        const std::string cmIndex = par.db1 + ".index";
        DBReader<unsigned int> cmReader(par.db1.c_str(),
                                        cmIndex.c_str(),
                                        1,
                                        DBReader<unsigned int>::USE_DATA | DBReader<unsigned int>::USE_INDEX);
        cmReader.open(DBReader<unsigned int>::NOSORT);
        queryModels.reserve(cmReader.getSize());
        for (size_t i = 0; i < cmReader.getSize(); ++i) {
            QueryModel qm;
            qm.key = cmReader.getDbKey(i);
            qm.useInfernalCompat = false;
            const char *raw = cmReader.getData(i, 0);
            size_t len = cmReader.getEntryLen(i);
        ...
        if (looksLikeInfernalCm(par.db1)) {
            ...
            qm.exactModel = parseInfernalCmExactModel(par.db1);
            qm.useInfernalCompat = true;
        } else {
            qm.g = parseGrammar(par.db1);
        }
        queryModels.push_back(qm);
    }

    std::vector<FastaSeq> seqs = hasDbIndex(par.db2) ? readSequenceDb(par.db2) : readFasta(par.db2);
    size_t observedDbResidues = 0;
    for (size_t i = 0; i < seqs.size(); ++i) {
        observedDbResidues += seqs[i].seq.size();
    }
    ...
    const bool wantInsideUser = (par.cmMode == Parameters::CM_MODE_INSIDE);
    std::unordered_map<unsigned int, size_t> seqIndexByKey;
    seqIndexByKey.reserve(seqs.size() * 2 + 1);
    for (size_t i = 0; i < seqs.size(); ++i) {
        seqIndexByKey[seqs[i].key] = i;
    }
    std::unordered_map<unsigned int, std::unordered_set<unsigned int> > candidatesByQuery
        = loadResultDbCandidates(par.db3, par.threads);
    ...
    size_t totalHits = 0;
    for (size_t qi = 0; qi < queryModels.size(); ++qi) {
        const QueryModel &qm = queryModels[qi];
        ...
        targetIds.reserve(cq->second.size());
        for (std::unordered_set<unsigned int>::const_iterator it = cq->second.begin(); it != cq->second.end(); ++it) {
            std::unordered_map<unsigned int, size_t>::const_iterator si = seqIndexByKey.find(*it);
            if (si != seqIndexByKey.end()) {
                targetIds.push_back(si->second);
            }
        }
        if (targetIds.empty()) {
            resultWriter.writeData("", 0, qm.key, 0);
        ...
        std::vector<Hit> hits;
        hits.reserve(1024);
        const bool wantInside = wantInsideUser || qm.useInfernalCompat;
        ...
        std::vector< std::vector<Hit> > perSeqHits(targetIds.size());
        if (nThreads <= 1 || targetIds.size() <= 1) {
            for (size_t i = 0; i < targetIds.size(); ++i) {
                const size_t sid = targetIds[i];
                if (qm.useInfernalCompat) {
                    runInfernalExactScan(qm.exactModel, seqs[sid].seq, wantInside, perSeqHits[i], seqs[sid].id);
                } else {
                    runDpScan(qm.g, seqs[sid].seq, wantInside, par.addBacktrace, perSeqHits[i], seqs[sid].id);
                }
            }
        } else {
            #pragma omp parallel for schedule(dynamic, 1) num_threads(static_cast<int>(nThreads))
            for (long i = 0; i < static_cast<long>(targetIds.size()); ++i) {
                const size_t sid = targetIds[static_cast<size_t>(i)];
                if (qm.useInfernalCompat) {
                    runInfernalExactScan(qm.exactModel, seqs[sid].seq, wantInside, perSeqHits[i], seqs[sid].id);
                } else {
                    runDpScan(qm.g, seqs[sid].seq, wantInside, par.addBacktrace, perSeqHits[i], seqs[sid].id);
        ...
        for (size_t i = 0; i < targetIds.size(); ++i) {
            ...
            for (size_t j = 0; j < perSeqHits[i].size(); ++j) {
                perSeqHits[i][j].dbKey = seqs[sid].key;
                perSeqHits[i][j].dbLen = static_cast<unsigned int>(seqs[sid].seq.size());
                perSeqHits[i][j].targetIdx = sid;
            }
        ...
        for (size_t i = 0; i < hits.size(); ++i) {
            ...
            if (qm.useInfernalCompat && hasBacktrace && hits[i].traceStates.empty() == false && hits[i].traceStates != "NA") {
                if (hits[i].targetIdx < seqs.size() && hits[i].start1 > 0 && hits[i].end1 >= hits[i].start1) {
                    const FastaSeq &fs = seqs[hits[i].targetIdx];
    ...
    headerWriter.open();
    for (size_t i = 0; i < seqs.size(); ++i) {
        headerWriter.writeData(seqs[i].id.c_str(), seqs[i].id.size(), seqs[i].key, 0);
    }
    headerWriter.close();
